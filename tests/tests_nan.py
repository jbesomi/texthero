"""
Test whether all functions can handle NaNs.
"""
...
test_cases_preprocessing = [
    ...
    ["phrases", preprocessing.phrases, (s_nan,)],
...
class AbstractNaNTest(PandasTestCase):
    ...
    def test_ignores_nan(self, name, test_function, valid_input):
        # Only have to check if no exception is raised. Everything
        # else is already taken care of by tests in test_helpers.py
        s = valid_input[0]
        try:
            test_function(*valid_input)
        except:
            self.fail("Exception was raised when NaNs are in input.")
