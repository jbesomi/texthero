...
- DataFrame: DataFrame is sparse and every column
...
class HeroTypes(pd.Series, pd.DataFrame):
    ...
    4. DataFrame: A DataFrame is a sparse Pandas DataFrame representing a matrix.
...
class DataFrame(HeroTypes):
    """
    A DataFrame is a sparse Pandas DataFrame
    ...
    def check_type(df: pd.DataFrame, input_output="") -> Tuple[bool, str]:
        """
        Check if a given intput has the properties of a Pandas DataFrame.
        """

        error_string = (
            "should be DataFrame: The input should be a DataFrame, so a sparse Pandas DataFrame"
